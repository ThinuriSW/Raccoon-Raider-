...
class RaccoonRaiders:
    ...
    def __init__(self, w: int, h: int, board_string: str = '') -> None:
        ...
        if board_string:
            ...
            populate_board(self._board, NUM_RACCOONS, NUM_GARBAGE_CANS, NUM_RECYCLING_BINS)

        self.square_size = min(int(SCREEN_WIDTH / w), int(SCREEN_HEIGHT / h))

        # Initialize a window of these pixel dimensions for display
        self._screen = pygame.display.set_mode((w * self.square_size, h * self.square_size))
    ...
    def draw(self) -> None:
        ...
        for y in range(len(state[0])):
            for x in range(len(state)):
                ...
                self._screen.blit(
                    self._background_tile, (x * self.square_size, y * self.square_size)
                )
    ...
    def play(self) -> None:
        ...
        # now print the text
        text_surface = font.render(f"Your Score: {score}", False, (0, 0, 0))
        self._screen.blit(text_surface, dest=(0, (self.square_size * self.height) // 2))
        pygame.display.flip()
...
# in order to work.
def populate_board(board: a1.GameBoard, num_raccoons: int, num_cans: int, num_bins: int) -> None:
